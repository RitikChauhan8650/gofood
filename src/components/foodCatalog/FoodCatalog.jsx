...
const FoodCatalog = () => {
  ...
  const foodEndpoint = location.pathname.split("/")[2]; //foods/burger
  const { token } = useSelector((state) => state.auth);

  useEffect(() => {
    const fetchFoodType = async () => {
      const res = await fetch(
        `http://localhost35000/product?category=${foodEndpoint}`,
        {
          headers: {
    ...
    fetchFoodType(); //call the function to get
  }, [foodEndpoint, token]);
            ...
            filteredFoods.map((f) => (
              ...
              <Link to={`/food/${f._id}`} key={f._id} className={classes.food}>
                <div className={classes.imgContainer}>
                  <img
                    src={`http://localhost:3001/images/${f?.img}`}
